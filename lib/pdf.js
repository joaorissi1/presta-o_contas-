...
export async function gerarPDF({ codigo, funcionarioNome, projeto, periodoIni, periodoFim, itens, contaIndex }) {
  ...
  itens.forEach((it) => {
    doc.addPage();
    doc.setFillColor(...INK); doc.rect(0, 0, W, mm(12), "F");
    doc.setTextColor(255, 255, 255); doc.setFont("helvetica", "bold"); doc.setFontSize(11);
    doc.text("RELATÓRIO DE DESPESAS", M, mm(8));
    doc.setFont("helvetica", "normal"); doc.setFontSize(9);
    doc.text(rel.codigo, W - M, mm(8), { align: "right" });
    let py = mm(24);
    doc.setTextColor(...INK); doc.setFont("helvetica", "bold"); doc.setFontSize(16);
    doc.text(it.tipo, M, py); py += mm(7);
    doc.setFont("helvetica", "normal"); doc.setFontSize(11); doc.setTextColor(90, 90, 90);
    doc.text(it.comentario || "", M, py, { maxWidth: W - 2 * M }); py += mm(11);
    doc.setFillColor(246, 244, 240); doc.rect(M, py, W - 2 * M, mm(18), "F");
    const colW = (W - 2 * M) / 3;
    [["FUNCIONÁRIO", rel.funcionario], ["DATA", dBR(it.data)],
      ["PAGO POR", it.destino === "Empresa" ? "Empresa" : "Funcionário (a reembolsar)"]].forEach((kv, i) => {
        const x = M + i * colW + mm(3);
        doc.setFont("helvetica", "normal"); doc.setFontSize(8); doc.setTextColor(130, 130, 130);
        doc.text(kv[0], x, py + mm(6));
        doc.setFont("helvetica", "bold"); doc.setFontSize(9.5); doc.setTextColor(...INK);
        doc.text(String(kv[1]), x, py + mm(13), { maxWidth: colW - mm(6) });
      });
    py += mm(26);
    doc.setFillColor(...INK); doc.rect(M, py, W - 2 * M, mm(8), "F");
    doc.setTextColor(255, 255, 255); doc.setFont("helvetica", "bold"); doc.setFontSize(9);
    doc.text("Nome da despesa", M + mm(2), py + mm(5.5));
    doc.text("Qtde", M + mm(105), py + mm(5.5), { align: "right" });
    doc.text("Subtotal", W - M - mm(2), py + mm(5.5), { align: "right" });
    py += mm(8);
    doc.setTextColor(...INK); doc.setFont("helvetica", "normal"); doc.setFontSize(10);
    doc.text((it.comentario || it.tipo).slice(0, 50), M + mm(2), py + mm(6));
    doc.text(String(it.prof || 1), M + mm(105), py + mm(6), { align: "right" });
    doc.text(brl(it.valor), W - M - mm(2), py + mm(6), { align: "right" });
    py += mm(14);
    doc.setFillColor(...GOLD); doc.rect(M + mm(100), py, W - 2 * M - mm(100), mm(10), "F");
    doc.setFont("helvetica", "bold"); doc.setFontSize(12);
    doc.text("Total:", M + mm(105), py + mm(7));
    doc.text(brl(it.valor), W - M - mm(2), py + mm(7), { align: "right" });
  });

  /* merge dos comprovantes */
  ...
  for (const it of itens) {
    ...
    try {
      ...
      if (it.anexoData.indexOf("data:application/pdf") === 0) {
        ...
        (await fim.copyPages(src, src.getPageIndices())).forEach((p) => fim.addPage(p));
      } else if (/^data:image\/(jpe?g)/.test(it.anexoData)) {
        const img = await fim.embedJpg(bytes); const pg = fim.addPage([595.28, 841.89]);
        const d = img.scaleToFit(515, 760);
        pg.drawImage(img, { x: (595.28 - d.width) / 2, y: (841.89 - d.height) / 2, width: d.width, height: d.height });
